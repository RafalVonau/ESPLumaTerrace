...
time_t calculate(double latitude, double longitude, int isSunrise, double zenith, struct tm *date) 
{
    ...
    double approxTimeOfEventInDays = isSunrise ?
...
uint64_t calcTimeToSleep(bool wifiFail) 
{
    ...
    if (update_alarm) {
        ...
        if (gled) {
            gled = 0;
            while (sunset <= tmp) sunset += (24u * 3600u);
            tmp = sunset - tmp;
            ...
            gled = 1;
            while (time_1am <= tmp) time_1am += (24u * 3600u);
            tmp = time_1am - tmp;
...
void goToDeepSleep(bool wifiFail = false)
{
	...
	if (sleepTimeSeconds < 5) sleepTimeSeconds = 5;
